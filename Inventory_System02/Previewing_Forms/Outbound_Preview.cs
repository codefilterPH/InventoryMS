...
{
    public partial class Outbound_Preview : Form
    {
            ...
            )
        {
            ...
            warranty = rec_warranty;
        }

        private void Outbound_Preview_Load(object sender, EventArgs e)
        {
            if (id != string.Empty)
            {
                txt_id.Text = id;
                ...
                txt_warranty.Text = warranty;

                txt_name.Focus();

                SQLConfig config = new SQLConfig();
                string sql = "SELECT Status FROM `Stock Out` WHERE `Stock ID` = '" + id + "' and `Transaction Reference` = '" + trans_ref + "' ";
